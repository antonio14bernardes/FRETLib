        ...
        ui.horizontal(|ui| {
            ...
            if ui.checkbox(&mut active, label).changed() {
                if active {
                    ...
                    // Special case for "Mean Total Intensity"
                    if label == "Mean Total Intensity" {
                        *filter_option = Some(Comparison::WithinNStd { n: 0 });
            ...
            if active {
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Min), |ui| {
                    if let Some(ref mut comparison) = filter_option {
                        ...
                        // If this is the "Mean Total Intensity" field, always use WithinNStd
                        if label == "Mean Total Intensity" {
                            // No combo box, always WithinNStd
                            let selected = "WithinNStd";
                            ui.label(selected); // Just display the text "WithinNStd"
                                ...
                                && (response.lost_focus() || ctx.input(|i| i.key_pressed(egui::Key::Enter)))
                            {
                                match input_buffer.parse::<T>() {
                                    Ok(parsed_value) => {
                                        *comparison = Comparison::WithinNStd { n: parsed_value.to_string().parse().unwrap_or_default() };
                                    }
                                    Err(_) => {
                            ...
                            // For other fields, show the combo box
